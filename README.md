...
Para este sistema decidí no utilizar soluciones comerciales ni fragmentadores estáticos por longitud de caracteres; en su lugar, implementé un algoritmo basado en similitud vectorial que se integra directamente con mi propio tokenizador [bpe-tokenizer-from-scratch](file:///Users/golfeno/Desarrollo/ai-core-infra/bpe-tokenizer-from-scratch) para validar los límites de capacidad física de contexto.
...
3. **Cálculo de Embeddings y Distancia Semántica:** Se obtienen los vectores de embeddings de las oraciones contextualizadas. La transición semántica se evalúa calculando la distancia del coseno ($1 - \text{similitud}$) entre oraciones consecutivas.
4. **Puntos de Corte Dinámicos (Boundaries):** Se establece un umbral de corte adaptativo utilizando la fórmula matemática:
   $$\text{Umbral} = \text{Media}(\text{Distancias}) + \text{threshold\_factor} \times \text{Desviación Estándar}(\text{Distancias})$$
   Cualquier distancia consecutiva que supere este umbral denota una ruptura temática y se marca como límite de chunk.
5. **Restricción de Tokens Integrada (Interlinking):** Para asegurar que ningún fragmento exceda la ventana máxima del LLM (`max_tokens`), cada bloque semántico se pre-tokeniza utilizando el [BPETokenizer](file:///Users/golfeno/Desarrollo/ai-core-infra/bpe-tokenizer-from-scratch/tokenizer.py) de mi primer proyecto. Si un bloque es excesivamente largo, se divide de manera secuencial a nivel de oraciones.
...
- Consume directamente las capacidades de conteo de [bpe-tokenizer-from-scratch](file:///Users/golfeno/Desarrollo/ai-core-infra/bpe-tokenizer-from-scratch) para respetar la ventana de contexto.
- En fases posteriores de la infraestructura, los embeddings serán generados por mi propio modelo desarrollado en [contrastive-embedding-trainer](file:///Users/golfeno/Desarrollo/ai-core-infra/contrastive-embedding-trainer).
- Los chunks limpios resultantes serán indexados y consultados en mi base de datos vectorial [nano-vector-db](file:///Users/golfeno/Desarrollo/ai-core-infra/nano-vector-db) como parte del pipeline de recuperación para la aplicación final [nexus-second-brain](file:///Users/golfeno/Desarrollo/ai-core-infra/nexus-second-brain).
